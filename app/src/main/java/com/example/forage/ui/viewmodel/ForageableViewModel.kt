 ...
 * and [AddForageableFragment] and allow for interaction to the [ForageableDao]
 */

//  pass a ForageableDao value as a parameter to the view model constructor
class ForageableViewModel(private val foragaeableDao: ForageableDao): ViewModel() {

    //  create a property to set to a list of all forageables from the DAO
    val allForageables: LiveData<List<Forageable>> = foragaeableDao.getForageables().asLiveData()
...
*/
    fun getForageable(id : Long) : LiveData<Forageable> {
        return foragaeableDao.getForageable(id).asLiveData()
    }
    ...
    // launch a coroutine and call the DAO method to add a Forageable to the database within it
        viewModelScope.launch {
            foragaeableDao.insert(forageable)
        }
    ...
    fun updateForageable(
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            foragaeableDao.update(forageable)
        }
    }
    ...
    fun deleteForageable(forageable: Forageable) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            foragaeableDao.delete(forageable)
        }
    }
